...
interface GridComponent<T extends React.ElementType = "div"> extends React.ForwardRefExoticComponent<
  Omit<React.ComponentPropsWithoutRef<T>, "ref"> & GridProps & React.RefAttributes<HTMLDivElement>
> {
  Col: typeof GridCol;
}

const Grid = React.forwardRef<HTMLDivElement, GridProps>(
  ({ children, gutter = 10, columns = 12, overflow = "visible", justify, align, className }, ref) => {
    useXbeshProviderCheck();
    const gutterValue = typeof gutter === "number" ? gutter : getValueForViewport(gutter, theme);
...
const GridCol = React.forwardRef<HTMLDivElement, GridColProps>(
  ({ children, span, offset = 0, order = 0, style, className }, ref) => {
    ...
    const colStyle: React.CSSProperties = {
      ...
      marginLeft: `${offsetValue * theme.gutterBase}px`,
      order: orderValue,
      ...style,
    };

    return (
      <div style={colStyle} ref={ref} className={className}>
        {children}
      </div>
    );
  }
